<template>
  <main>
    ...
    <div v-else-if="error" class="msg err">
      <p>{{ error }}</p>
      <button @click="fetchMenu">Повторить</button>
    </div>
    ...
    <template v-else>
  <div class="grid">
    <!-- ✅ ТОЛЬКО ОДИН v-for: используем paginatedItems -->
    <div v-for="item in paginatedItems" :key="item.id" class="card">
      ...
      <div class="actions-row">
        ...
        >
          <!-- 🔴 Красное сердце (если лайкнуто) -->
          <svg 
            v-if="likesState[item.id]"
          ...
          <!-- 🟡 Жёлтое сердце (если не лайкнуто) -->
          <svg 
            v-else
            class="heart-icon"
            viewBox="0 0 22 22" 
  ...
  <!-- 🔥 КНОПКИ ПАГИНАЦИИ (ВНЕ .grid и вне v-for!) -->
...
const loadProductStates = async () => {
  ...
  if (!products.value?.length) {
    console.log('Товары ещё не загружены, пропускаем...')
    return
  }
  
  console.log(`Загружаем состояния для ${products.value.length} товаров...`)
  
  // Создаём Promise для всех запросов
  const promises = products.value.map(async (item) => {
    try {
  ...
  params: { user_id: auth.user.id }  // ← Добавь эту строку!
      ...
      likesCount.value[item.id] = likeRes.data.likes || item.likes || 0
      
      // Загружаем статус корзины
      const cartRes = await api.get(`/cart/${item.id}/check`)
      cartState.value[item.id] = cartRes.data.in_cart
      
      console.log(`✓ Товар ${item.id}: лайк=${likeRes.data.is_liked}, корзина=${cartRes.data.in_cart}`)
    } catch (e) {
      console.error(`❌ Ошибка для товара ${item.id}:`, e.message)
      // Устанавливаем дефолтные значения
      likesState.value[item.id] = false
      likesCount.value[item.id] = item.likes || 0
      cartState.value[item.id] = false
    }
  })
  
  // Ждём выполнения всех запросов
  await Promise.all(promises)
  console.log('✅ Все состояния загружены!')
}

// Следим за авторизацией
watch(() => auth.isAuthenticated, (val) => { 
  console.log('Авторизация изменилась:', val)
  if (val) {
    setTimeout(() => loadProductStates(), 100) // Небольшая задержка
  } else {
...
// Следим за загрузкой товаров
watch(() => products.value?.length, (newLength) => { 
  console.log('Количество товаров:', newLength)
  if (newLength > 0 && auth.isAuthenticated) {
    setTimeout(() => loadProductStates(), 100) // Небольшая задержка
  }
})
...
onMounted(() => {
  document.title = 'Меню | Bon\'s Burgers'
...
<style scoped src="@/assets/styles/views/Menu.css">
